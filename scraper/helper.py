...
def getDriver(url=''):
    ...
    printMessage('DEBUG', 'helper.getDriver', 'Setting up options for Chromium browser ...\n')
    options = webdriver.ChromeOptions()
    printMessage('DEBUG', 'helper.getDriver', f'\tSetting headless mode to {C.HEADLESS_MODE}')
    if C.HEADLESS_MODE:
        options.add_argument('--headless')
    options.timeouts = {'pageLoad': C.LOADING_TIMEOUT, 'implicit': 60000}
    options.add_argument("--window-size=1920,1080")
    printMessage('DEBUG', 'helper.getDriver', 'Launching instance of Chromium browser ...\n')
    driver = webdriver.Chrome(options=options)
    if not C.HEADLESS_MODE:
        driver.maximize_window()
    if url == '' or url == None:
        printMessage('DEBUG', 'helper.getDriver', 'Returning a driver with an empty url.\n')
        return driver
    printMessage('DEBUG', 'helper.getDriver', f'Loading web address to driver: {url.replace(C.SITE_INDEX, "[held]")}')
    try:
        driver.get(url)
        printMessage('DEBUG', 'helper.getDriver', 'Chromium driver instance is setup and ready.')
    except:
        printMessage('ERROR', 'helper.getDriver', 'Could not load address to Chromium driver.')
        traceback.print_exc()
    return driver
...
def sleepRandom(Fm=55, To=115):
    ...
    printMessage('DEBUG', 'helper.sleepRandom', f'ZZZZZZZZZZZZZZZZZZZZZZZZZZZ. Sleeping for a ({rint}s) while.\n\n')
    time.sleep(1)
    time.sleep(rint)
    return 0
...
def cleanEmptyDceFiles(dry_run=True, base_folder=''):
    ...
    nf, nd = 0, 0
    printMessage('INFO', 'helper.cleanEmptyDceFiles', f"Deleting empty files from: {base_folder}")
    ...
    for subfolder in os.listdir(base_folder):
        ...
        if os.path.isdir(subfolder_path) and subfolder.startswith("DCE-"):
            nd += 1
            # printMessage('DEBUG', 'helper.cleanEmptyDceFiles', f"Browsing folder: {subfolder_path}")
            ...
            for file_name in os.listdir(subfolder_path):
                ...
                if os.path.isfile(file_path) and os.path.getsize(file_path) == 0:
                    ...
                    if dry_run: 
                        printMessage('DEBUG', 'helper.cleanEmptyDceFiles', f"DRY-RUN = empty file: {file_path}")
                    else:   
                        printMessage('DEBUG', 'helper.cleanEmptyDceFiles', f"Deleting empty file: {file_path}")
                        os.remove(file_path)
                    nf += 1
    printMessage('INFO', 'helper.cleanEmptyDceFiles', f"Subfolders scanned: {nd}, Files affected: {nf}")
