...
def getFirstPage():
    ...
    try:
        ...
        helper.printMessage('ERROR', 'linker.getFirstPage', f'Error trimming UA: {str(ve)}')
    
    helper.printMessage('DEBUG', 'linker.getFirstPage', f'Using UA: {rua_label}.')
    headers = {"User-Agent": rua }



    # get >> /index.php?page=entreprise.EntrepriseAdvancedSearch&searchAnnCons
    # post > /index.php?page=entreprise.EntrepriseAdvancedSearch&searchAnnCons

    url = "?page=entreprise.EntrepriseAdvancedSearch&searchAnnCons"
    full_url = f"{C.SITE_INDEX}{url}"
    helper.printMessage('DEBUG', 'linker.getFirstPage', f'Getting page at {C.SITE_INDEX}{url}')
    session = requests.Session()
    response_get = session.get(full_url, headers=headers, timeout=C.REQ_TIMEOUT, allow_redirects=True)
    print("\n========== Status Code:", response_get.status_code)

    with open(f"exports/get-{response_get.status_code}.html", 'w', newline='') as file:
        file.write(response_get.text)

    cookies = session.cookies.get_dict()
    print("\n========== Cookies from GET response:\n", cookies)

    soup = BeautifulSoup(response_get.text, "html.parser")
    form = soup.find("form", id="ctl0_ctl2")  # Replace 'myForm' with the form's ID or class

    payload = {}
    helper.printMessage('DEBUG', 'linker.getFirstPage', f'Parsing the GET response ... ')
    if form:
        inputs = form.find_all("input")
        for input_element in inputs:
            input_name = input_element.get("name")
            input_value = input_element.get("value", "")  # Use empty string if no value
            if input_name:  # Only include inputs with a name attribute
                payload[input_name] = input_value

    helper.printMessage('DEBUG', 'linker.getFirstPage', f'Payload read: {payload}', 1, 5)
    helper.printMessage('DEBUG', 'linker.getFirstPage', 'Posting back the form ... ', 1, 5)
    response_post = session.post(full_url, data=payload, headers=headers)

    print("\n========== Status Code:", response_post.status_code)

    with open(f"exports/post-{response_post.status_code}.html", 'w', newline='') as file:
        file.write(response_post.text)

    cookies = session.cookies.get_dict()
    print("\n========== Cookies from POST response:\n", cookies)

    soup = BeautifulSoup(response_post.text, "html.parser")


    results = soup.find("table", class_="table-results")
    # if results.find_all("tr"):
    links = []


    if results:
        lines = results.find_all("tr")
        if lines:
            for line in lines:
                cat = line.find("div", id="ctl0_CONTENU_PAGE_resultSearch_tableauResultSearch_ctl4_panelBlocCategorie")
                if cat:
                    published_el = cat.find_next_sibling("div")
                pan = line.find("div", id="ctl0_CONTENU_PAGE_resultSearch_tableauResultSearch_ctl3_panelAction")
                if pan:
                    button_el = pan.find("a")
                    if button_el:
                        drat = button_el.get_attribute("href").replace(C.LINK_PREFIX, '') #### 825152&orgAcronyme=q9t
                        if drat:
                            portal_id_text = drat.split(C.LINK_STITCH)[0]
                            organism_text = drat.split(C.LINK_STITCH)[1]
                if drat and published_el:
                    link = [portal_id_text, organism_text, published_el.get_attribute("innerText").strip()]
                    links.append(link)
                    print(f"\n\n =============== {link} ===============")
    else:
        helper.printMessage('ERROR', 'linker.getFirstPage', 'Did not find the results table', 1, 5)
